            ...
            if ( name && email && message ) {
                 $.ajax({
                     ...
                     success:function(data){
                         ...
                         $('#contact_form_submit').prepend('<span className="alert alert-success email-success contact-alert">'+data+'</span>');
                         $('#contact-name').val('');
                         $('#contact-email').val('');
                         $('#contact-message').val('');
                ...
                $('#contact_form_submit').children('.email-success').remove();
                $('#contact_form_submit').prepend('<span className="alert alert-danger email-success contact-alert">Somenthing went wrong</span>');
                $('.email-success').fadeOut(3000);
             }
        });
}(jQuery));